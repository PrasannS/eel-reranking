...
class LatticeBertSelfAttention(nn.Module):
...
class LatticeBertAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = LatticeBertSelfAttention(config)
...
class LatticeBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = LatticeBertAttention(config)
...
class LatticeBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([LatticeBertLayer(config) for _ in range(config.num_hidden_layers)])
...
class LatticeBertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = LatticeBertEncoder(config)
