...
def create_sortedbatch_data(lpair, scorer, tok, bsize=32):
    lcands = load_cands(exploded_dirs[lpair])
    cand_df = lset_tocominp(lcands, bsize)
    bdir = "torchsaved/"+str(lpair)+"train"
    # ensure that we're valid, load in old data if needed
    assert scorer=='cqe' or scorer=='comet'
